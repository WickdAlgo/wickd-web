 ...
 *
 * Structure geometry is *derived from the generated candles*, not typed in
 * beside them. A hand-written order block sits at whatever price someone
 * guessed; this one sits on the actual body of the actual candle it names, so
 * the overlay lands where a reader expects and the fixture cannot drift from
 * its own data.
 *
 * The narrative — indecision, swing high, equal highs, sweep, order block,
 * fair value gap — and its timestamps come from the shell this replaced. One
 * correction was made: that version listed the sweep *before* the equal-highs
 * pool it swept. Reversed here, because a causal replay renders cause before
 * effect and the old order would have shown a sweep of nothing.
 */
...
/** Index of the candle opening at a given UTC clock time on 2026-05-06. */
...
const I_IC = idx("12:50");
const I_SWING_HI = idx("13:05");
const I_SWING_HI_CONFIRMED = idx("13:25");
const I_EQH = idx("13:40");
const I_SWEEP = idx("13:55");
const I_OB = idx("14:20");
const I_OB_CONFIRMED = idx("14:25");
const I_FVG = idx("14:35");
const I_OB_MITIGATED = idx("14:55");
const I_SWEEP_REJECT = idx("15:10");

/** Lowest low in the session leading up to the swing high. */
const I_SWING_LO = candles
  .slice(0, I_SWING_HI)
  .reduce((lo, c, i, arr) => (c.low < arr[lo].low ? i : lo), 0);

const obCandle = candles[I_OB];
...
const eqhPrice = Math.max(candles[I_SWING_HI].high, candles[I_EQH].high);
...
const raw = {
  ...
  lifecycle: [
    ...
    { id: "lc-010", entityId: "ob-018", type: "touched", knownAtUtc: closeAt(I_OB_MITIGATED - 2), sequence: 0 },
  ...
  relations: [
    ...
    { id: "rel-002", fromEntityId: "eqh-001", toEntityId: "swing-hi-001", type: "caused_by" },
    { id: "rel-003", fromEntityId: "sweep-001", toEntityId: "eqh-001", type: "swept" },
    { id: "rel-004", fromEntityId: "ob-018", toEntityId: "sweep-001", type: "caused_by" },
    { id: "rel-005", fromEntityId: "fvg-042", toEntityId: "ob-018", type: "confirms" },
  ],
  layers: [
    {
      id: "order-blocks",
